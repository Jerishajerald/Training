...
//In the entity class there is property called firstName	
//  List<Driver> findByFirstName(String srchName);
 
  List<Driver> findByMobileNumber(long mobileNumber);

//we are going to use our query
  @Query(value="from Driver where rating =:search")
  List<Driver> searchByRating(@Param("search") double rating);
